...
SELECT
    ...
    ROUND((SUM(f.profit) / NULLIF(SUM(f.line_total),0)) * 100, 2) AS profit_margin_pct,
    SUM(f.quantity) AS units_sold
FROM warehouse.fact_sales f
JOIN warehouse.dim_products p
  ON f.product_key = p.product_key
...
SELECT
    ...
    p.category,
    SUM(f.profit) AS total_profit,
    ROUND((SUM(f.profit) / NULLIF(SUM(f.line_total),0)) * 100, 2) AS profit_margin,
...
SELECT
    ...
    ROUND(AVG(COUNT(*)) OVER (PARTITION BY d.day_name), 2) AS avg_daily_transactions,
    SUM(f.line_total) AS total_revenue
FROM warehouse.fact_sales f
JOIN warehouse.dim_date d
  ON f.date_key = d.date_key
